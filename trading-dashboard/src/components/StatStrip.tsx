...
export function StatStrip({
  ...
  // Win rate and profit factor are the two that carry a verdict, so they are
  // the only two allowed to take a semantic colour.
  ...
  const pf = stats?.pf_R;
  const pfNumber =
    typeof pf === "number" ? pf : pf?.profit_factor_R_infinite ? Infinity : pf?.profit_factor_R;
  if (pfNumber != null) {
